...
fn run_with(
    ...
    // Captured before any `set` mutates the file: sqlite here means a
    // re-run, whose embedder kind is a choice to keep, not bge_m3 by default.
    let sqlite_at_start = config.store.kind == StoreKind::Sqlite;
