...
Its own module so `tools.py` — the MCP surface — has no dependency on
`google-genai`. That keeps the SDK out of the import path of anything that
only needs to shape a tool result, which is both cleaner and noticeably faster
to start.
"""
...
class NewsToolError(Exception):
    ...
    Bad arguments, a refused URL scheme, an empty query: things the caller can
